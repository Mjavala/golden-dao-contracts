    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(        
        ...
        require(_gold != address(0),"initialize: Invalid address");
    ...
    function validatePayout() external{
        uint256 balance = IERC20Upgradeable(gold).balanceOf(address(this));
        uint256[] memory getgoldAmountOneETH = IUniswapV2Router(router).getAmountsOut(totalETH, path);
        uint256 totalAmount = getgoldAmountOneETH[1].mul(155).div(10);
        if(balance >= totalAmount && IGovernance(dao).nextInvesteeFund()<IGovernance(dao).nextInvestee()){
            fundInvestee(totalAmount,getgoldAmountOneETH[1].mul(25).div(10));
        }
    }
    ...
    function fundInvestee(uint256 totalAmount,uint burnAmount) internal nonReentrant{
        ...
        IERC20Upgradeable(gold).transfer(DEAD_ADDRESS,burnAmount);
        IERC20Upgradeable(gold).transfer(investee,totalAmount.sub(burnAmount));
    }
